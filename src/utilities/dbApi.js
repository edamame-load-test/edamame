...
const dbApi = {
  ...
  logUrl() {
    return kubectl.getIngress(DB_API_INGRESS_NAME).then(({ stdout }) => {
  ...
  newTestId(testPath, name) {
    ...
    return this.postRequest(testContent, url, name)
      .catch(() => {
        return this.restoreIp().then(() =>
          this.postRequest(testContent, this.url(), name)
        );
      })
      .then((res) => {
        return res.data.id;
  ...
  getTest(name) {
    return this.getAllTests()
      .catch(() => {
        return this.restoreIp().then(() => this.getAllTests());
      })
      .then((tests) => {
  ...
  updateTestStatus(id, status) {
    this.putRequest(id, { status }).catch(() => {
      return this.restoreIp().then(() => this.putRequest(id, { status }));
    });
  },
  ...
  deleteTest(id) {
    ...
    return axios
      .delete(`${url}/${id}`)
      .catch(() => {
        return this.restoreIp().then(() => axios.delete(`${this.url()}/${id}`));
      })
      .then((res) => {
        return res.status;
      });
  ...
  getAllTests() {
    ...
    return axios
      .get(url)
      .catch(() => {
        return this.restoreIp().then(() => axios.get(this.url()));
      })
      .then((res) => {
        return res.data;
      });
